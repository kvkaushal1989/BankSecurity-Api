...
namespace Repository.BankSecurity.MasterSetup {
    public class CMC_RemarkRepository : ICMC_RemarkRepository {
        #region Create New Row
        public Dictionary<string, object> CreateRow(CMC_Remark data) {
            Dictionary<string, object> ResponseData = new Dictionary<string, object>();
            DataSet DataSet = new DataSet();
            var InputParameters = new[] { data }.ToList();

            using (var DBManager = new DBManager(Connection.GetOperationConnection())) {
                DataSet = DBManager.Post("BS_CMC_RemarkCreate_PROC", InputParameters);
            }
        ...
        #region Update Row
        public Dictionary<string, object> UpdateRow(CMC_Remark data) {
            Dictionary<string, object> ResponseData = new Dictionary<string, object>();
            DataSet DataSet = new DataSet();
            var InputParameters = new[] { data }.ToList();

            using (var DBManager = new DBManager(Connection.GetOperationConnection())) {
                DataSet = DBManager.Put("BS_CMC_RemarkUpdate_PROC", InputParameters);
            }
        ...
        public Dictionary<string, object> DeleteRow(List<int> idArray) {
            ...
            using (var DBManager = new DBManager(Connection.GetOperationConnection())) {
                DataSet = DBManager.Post("BS_CMC_RemarkDelete_PROC", InputParameters);
            }
        ...
        public Dictionary<string, object> ViewRow(long id) {
            ...
            using (var DBManager = new DBManager(Connection.GetOperationConnection())) {
                DataSet = DBManager.GetDataSet("BS_CMC_RemarkViewRowByID_PROC", InputParameters);
            }

            Status Status = (DataSet.Tables[0]).ConvertToList<Status>()[0];

            if (Status.IsSuccess == true) {
                List<CMC_Remark> data = (DataSet.Tables[1]).ConvertToList<CMC_Remark>();
        ...
        #region Pagination
        public Dictionary<string, object> PaginationMessage(List<CMC_Remark> paginatedRowArray, PageLinkBuilder linkBuilder, int totalRecordCount) {
        ...
        /// <returns></returns>
        public List<CMC_Remark> PaginationSettings(string filter, string searchText, int pageSize, int pageNumber, ref int totalRecordCount, ref int totalPages, ref int skipRecords) {
            try {
        ...
        List<CMC_Remark> PaginationList(int skipRecords, int pageSize, string filter, string searchText) {
            try {
                DataSet DataSet = new DataSet();
                var InputParameters = new[] { new { PageSize = pageSize, PageNumber = skipRecords, Filter = filter, SearchText = searchText } }.ToList();

                using (var DBManager = new DBManager(Connection.GetOperationConnection())) {
                    DataSet = DBManager.GetDataSet("BS_CMC_RemarkPaginationList_PROC", InputParameters);
                }

                Status Status = (DataSet.Tables[0]).ConvertToList<Status>()[0];

                if (Status.IsSuccess == true) {
                    return DataSet.Tables[1].ConvertToList<CMC_Remark>();
                }
                else {
                    return null;
                }
        ...
        public int GetAllCount(string filter, string searchText) {
            try {
                ...
                using (var DBManager = new DBManager(Connection.GetOperationConnection())) {
                    DataSet = DBManager.GetDataSet("BS_CMC_RemarkPaginationListTotal_PROC", InputParameters);
                }
        ...
        public Dictionary<string, object> GetDropDownList() {
            ...
            using (var DBManager = new DBManager(Connection.GetOperationConnection())) {
                //DataSet = DBManager.GetDataSet("BS_CMC_RemarkDropDownList_PROC", InputParameters);
