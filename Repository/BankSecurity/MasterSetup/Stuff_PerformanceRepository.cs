...
namespace Repository.BankSecurity.MasterSetup {
    public class Stuff_PerformanceRepository : IStuff_PerformanceRepository {
        #region Create New Row
        public Dictionary<string, object> CreateRow(Stuff_Performance data) {
            Dictionary<string, object> ResponseData = new Dictionary<string, object>();
            DataSet DataSet = new DataSet();
            var InputParameters = new[] { data }.ToList();

            using (var DBManager = new DBManager(Connection.GetOperationConnection())) {
                DataSet = DBManager.Post("BS_Stuff_PerformanceCreate_PROC", InputParameters);
            }
        ...
        #region Update Row
        public Dictionary<string, object> UpdateRow(Stuff_Performance data) {
            Dictionary<string, object> ResponseData = new Dictionary<string, object>();
            DataSet DataSet = new DataSet();
            var InputParameters = new[] { data }.ToList();

            using (var DBManager = new DBManager(Connection.GetOperationConnection())) {
                DataSet = DBManager.Put("BS_Stuff_PerformanceUpdate_PROC", InputParameters);
            }
        ...
        public Dictionary<string, object> DeleteRow(List<int> idArray) {
            ...
            using (var DBManager = new DBManager(Connection.GetOperationConnection())) {
                DataSet = DBManager.Post("BS_Stuff_PerformanceDelete_PROC", InputParameters);
            }
        ...
        public Dictionary<string, object> ViewRow(long id) {
            ...
            using (var DBManager = new DBManager(Connection.GetOperationConnection())) {
                DataSet = DBManager.GetDataSet("BS_Stuff_PerformanceViewRowByID_PROC", InputParameters);
            }

            Status Status = (DataSet.Tables[0]).ConvertToList<Status>()[0];

            if (Status.IsSuccess == true) {
                List<Stuff_Performance> data = (DataSet.Tables[1]).ConvertToList<Stuff_Performance>();
        ...
        #region Pagination
        public Dictionary<string, object> PaginationMessage(List<Stuff_Performance> paginatedRowArray, PageLinkBuilder linkBuilder, int totalRecordCount) {
        ...
        /// <returns></returns>
        public List<Stuff_Performance> PaginationSettings(string filter, string searchText, int pageSize, int pageNumber, ref int totalRecordCount, ref int totalPages, ref int skipRecords) {
            try {
        ...
        List<Stuff_Performance> PaginationList(int skipRecords, int pageSize, string filter, string searchText) {
            try {
                DataSet DataSet = new DataSet();
                var InputParameters = new[] { new { PageSize = pageSize, PageNumber = skipRecords, Filter = filter, SearchText = searchText } }.ToList();

                using (var DBManager = new DBManager(Connection.GetOperationConnection())) {
                    DataSet = DBManager.GetDataSet("BS_Stuff_PerformancePaginationList_PROC", InputParameters);
                }

                Status Status = (DataSet.Tables[0]).ConvertToList<Status>()[0];

                if (Status.IsSuccess == true) {
                    return DataSet.Tables[1].ConvertToList<Stuff_Performance>();
                }
                else {
                    return null;
                }
        ...
        public int GetAllCount(string filter, string searchText) {
            try {
                ...
                using (var DBManager = new DBManager(Connection.GetOperationConnection())) {
                    DataSet = DBManager.GetDataSet("BS_Stuff_PerformancePaginationListTotal_PROC", InputParameters);
                }
        ...
        public Dictionary<string, object> GetDropDownList() {
            ...
            using (var DBManager = new DBManager(Connection.GetOperationConnection())) {
                //DataSet = DBManager.GetDataSet("BS_Stuff_PerformanceDropDownList_PROC", InputParameters);
