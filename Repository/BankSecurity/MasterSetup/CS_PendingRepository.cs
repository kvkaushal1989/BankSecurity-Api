...
namespace Repository.BankSecurity.MasterSetup {
    public class CS_PendingRepository : ICS_PendingRepository {
        #region Create New Row
        public Dictionary<string, object> CreateRow(CS_Pending data) {
            Dictionary<string, object> ResponseData = new Dictionary<string, object>();
            DataSet DataSet = new DataSet();
            var InputParameters = new[] { data }.ToList();

            using (var DBManager = new DBManager(Connection.GetOperationConnection())) {
                DataSet = DBManager.Post("BS_CS_PendingCreate_PROC", InputParameters);
            }
        ...
        #region Update Row
        public Dictionary<string, object> UpdateRow(CS_Pending data) {
            Dictionary<string, object> ResponseData = new Dictionary<string, object>();
            DataSet DataSet = new DataSet();
            var InputParameters = new[] { data }.ToList();

            using (var DBManager = new DBManager(Connection.GetOperationConnection())) {
                DataSet = DBManager.Put("BS_CS_PendingUpdate_PROC", InputParameters);
            }
        ...
        public Dictionary<string, object> DeleteRow(List<int> idArray) {
            ...
            using (var DBManager = new DBManager(Connection.GetOperationConnection())) {
                DataSet = DBManager.Post("BS_CS_PendingDelete_PROC", InputParameters);
            }
        ...
        public Dictionary<string, object> ViewRow(long id) {
            ...
            using (var DBManager = new DBManager(Connection.GetOperationConnection())) {
                DataSet = DBManager.GetDataSet("BS_CS_PendingViewRowByID_PROC", InputParameters);
            }

            Status Status = (DataSet.Tables[0]).ConvertToList<Status>()[0];

            if (Status.IsSuccess == true) {
                List<CS_Pending> data = (DataSet.Tables[1]).ConvertToList<CS_Pending>();
        ...
        #region Pagination
        public Dictionary<string, object> PaginationMessage(List<CS_Pending> paginatedRowArray, PageLinkBuilder linkBuilder, int totalRecordCount) {
        ...
        /// <returns></returns>
        public List<CS_Pending> PaginationSettings(string filter, string searchText, int pageSize, int pageNumber, ref int totalRecordCount, ref int totalPages, ref int skipRecords) {
            try {
        ...
        List<CS_Pending> PaginationList(int skipRecords, int pageSize, string filter, string searchText) {
            try {
                DataSet DataSet = new DataSet();
                var InputParameters = new[] { new { PageSize = pageSize, PageNumber = skipRecords, Filter = filter, SearchText = searchText } }.ToList();

                using (var DBManager = new DBManager(Connection.GetOperationConnection())) {
                    DataSet = DBManager.GetDataSet("BS_CS_PendingPaginationList_PROC", InputParameters);
                }

                Status Status = (DataSet.Tables[0]).ConvertToList<Status>()[0];

                if (Status.IsSuccess == true) {
                    return DataSet.Tables[1].ConvertToList<CS_Pending>();
                }
                else {
                    return null;
                }
        ...
        public int GetAllCount(string filter, string searchText) {
            try {
                ...
                using (var DBManager = new DBManager(Connection.GetOperationConnection())) {
                    DataSet = DBManager.GetDataSet("BS_CS_PendingPaginationListTotal_PROC", InputParameters);
                }
        ...
        public Dictionary<string, object> GetDropDownList() {
            ...
            using (var DBManager = new DBManager(Connection.GetOperationConnection())) {
                //DataSet = DBManager.GetDataSet("BS_CS_PendingDropDownList_PROC", InputParameters);
