...
namespace Repository.BankSecurity.MasterSetup {
    public class Wem_PlayerRepository : IWem_PlayerRepository {
        #region Create New Row
        public Dictionary<string, object> CreateRow(Wem_Player data) {
            Dictionary<string, object> ResponseData = new Dictionary<string, object>();
            DataSet DataSet = new DataSet();
            var InputParameters = new[] { data }.ToList();

            using (var DBManager = new DBManager(Connection.GetOperationConnection())) {
                DataSet = DBManager.Post("BS_Wem_PlayerCreate_PROC", InputParameters);
            }
        ...
        #region Update Row
        public Dictionary<string, object> UpdateRow(Wem_Player data) {
            Dictionary<string, object> ResponseData = new Dictionary<string, object>();
            DataSet DataSet = new DataSet();
            var InputParameters = new[] { data }.ToList();

            using (var DBManager = new DBManager(Connection.GetOperationConnection())) {
                DataSet = DBManager.Put("BS_Wem_PlayerUpdate_PROC", InputParameters);
            }
        ...
        public Dictionary<string, object> DeleteRow(List<int> idArray) {
            ...
            using (var DBManager = new DBManager(Connection.GetOperationConnection())) {
                DataSet = DBManager.Post("BS_Wem_PlayerDelete_PROC", InputParameters);
            }
        ...
        public Dictionary<string, object> ViewRow(long id) {
            ...
            using (var DBManager = new DBManager(Connection.GetOperationConnection())) {
                DataSet = DBManager.GetDataSet("BS_Wem_PlayerViewRowByID_PROC", InputParameters);
            }

            Status Status = (DataSet.Tables[0]).ConvertToList<Status>()[0];

            if (Status.IsSuccess == true) {
                List<Wem_Player> data = (DataSet.Tables[1]).ConvertToList<Wem_Player>();
        ...
        #region Pagination
        public Dictionary<string, object> PaginationMessage(List<Wem_Player> paginatedRowArray, PageLinkBuilder linkBuilder, int totalRecordCount) {
        ...
        /// <returns></returns>
        public List<Wem_Player> PaginationSettings(string filter, string searchText, int pageSize, int pageNumber, ref int totalRecordCount, ref int totalPages, ref int skipRecords) {
            try {
        ...
        List<Wem_Player> PaginationList(int skipRecords, int pageSize, string filter, string searchText) {
            try {
                DataSet DataSet = new DataSet();
                var InputParameters = new[] { new { PageSize = pageSize, PageNumber = skipRecords, Filter = filter, SearchText = searchText } }.ToList();

                using (var DBManager = new DBManager(Connection.GetOperationConnection())) {
                    DataSet = DBManager.GetDataSet("BS_Wem_PlayerPaginationList_PROC", InputParameters);
                }

                Status Status = (DataSet.Tables[0]).ConvertToList<Status>()[0];

                if (Status.IsSuccess == true) {
                    return DataSet.Tables[1].ConvertToList<Wem_Player>();
                }
                else {
                    return null;
                }
        ...
        public int GetAllCount(string filter, string searchText) {
            try {
                ...
                using (var DBManager = new DBManager(Connection.GetOperationConnection())) {
                    DataSet = DBManager.GetDataSet("BS_Wem_PlayerPaginationListTotal_PROC", InputParameters);
                }
        ...
        public Dictionary<string, object> GetDropDownList() {
            ...
            using (var DBManager = new DBManager(Connection.GetOperationConnection())) {
                //DataSet = DBManager.GetDataSet("BS_Wem_PlayerDropDownList_PROC", InputParameters);
